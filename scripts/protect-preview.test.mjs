...
test('branch and PR builds add no-index rules without publishing credentials', async (t) => {
  for (const context of ['branch-deploy', 'deploy-preview']) {
    ...
    assert.ok(!headers.includes('Basic-Auth:'));
    assert.ok(!headers.includes(credentials));
    assert.equal(headers.split('# CSE preview protection').length, 2);

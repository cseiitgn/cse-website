"""Three original Matplotlib announcement layouts. Photo pixels are not retouched.
...
def gaussian(ax,c,s):
    ...
    for depth,xy,vals,ang,u in sorted(samples,key=lambda x:x[0]):
        mix=(np.sin(u+.5)+1)/2
        color=np.array(matplotlib.colors.to_rgb('#0f7e84'))*(1-mix)+np.array(matplotlib.colors.to_rgb('#c1783f'))*mix
    ...
    for i in range(nu):
      for j in range(nv):
        ...
        for ids in [(0,1,2),(0,2,3)]:
            ...
            t=(np.sin(u+.45)+1)/2
            col=(np.array(matplotlib.colors.to_rgb('#44bac1'))*(1-t)+np.array(matplotlib.colors.to_rgb('#ecad70'))*t)*shade
            faces.append((np.mean(pts@R.T,axis=0)[2],np.array(c)+pts@P.T*s,col))
    for z,xy,col in sorted(faces,key=lambda x:x[0]):
        ax.add_patch(Polygon(xy,facecolor=col,edgecolor='#143944',linewidth=.35))
...
def wire(ax,c,s):
    for v in np.linspace(0,2*np.pi,7,endpoint=False):
        ...
        ax.plot(pts[:,0],pts[:,1],color='#2a797c',alpha=.58,lw=.6)
    for u in np.linspace(0,2*np.pi,16,endpoint=False):
        ...
        ax.plot(pts[:,0],pts[:,1],color='#2a797c',alpha=.38,lw=.5)
...
class Poster:
    ...
    def header(self):
        ...
        self.text(1012,52,'COMPUTER SCIENCE\n& ENGINEERING',18,bold=True,ha='right',color='#183a3c',linespacing=1.4)
...
def render(post,template):
    dark=template=='low-poly'
    p=Poster(f"{post['slug']}/{template}", '#102f38' if dark else '#ffffff' if template=='simple' else '#f5f3ec', '#f3f3e9' if dark else '#183a3c', '#b6cacc' if dark else '#536663', '#72d5d4' if dark else '#147b83',dark)
    p.post=post
